...
{#if nodeDetails}
	...
	>
		<!-- close button -->
		<button
			onclick={() => globalState.setSelectedNodeNr(undefined)}
		...
		{#await nodeDetails then { split, alternativeSplits, heightDistribution }}
			<div class="flex flex-col gap-1">
				<div class="flex items-center gap-3">
					<h3 class="text-accent text-sm font-semibold uppercase">Selected Clade</h3>
					<span
						class="rounded-full border border-gray-200 bg-white px-2 py-1 text-xs font-semibold"
					>
